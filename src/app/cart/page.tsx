      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-100 pb-8">
        ...
        <div className="bg-white border border-gray-200 px-5 py-2.5 rounded-2xl shadow-sm flex items-center gap-3 self-start md:self-center">
          ...
          <p className="text-gray-700 font-bold text-sm md:text-base whitespace-nowrap">
            ...
            {(productData?.products?.length || 0) === 1
              ? " Item"
              : " Items"}{" "}
          </p>
        </div>
      </div>
      {(productData?.products?.length || 0) > 0 ? (
        <div className="my-5">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {productData?.products.map((item) => (
                <div
                  key={item._id}
                  className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center md:items-start gap-6 relative"
                >
                  {/* Product Image */}
                  ...
                  <div className="flex-1 text-center md:text-left w-full">
                    ...
                    <p className="text-green-600 font-extrabold text-lg mt-2 md:mt-3">
                      {item.price} EGP
                    </p>
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-0 border border-gray-200 rounded-xl w-full max-w-[140px] mx-auto md:mx-0 mt-4 bg-white overflow-hidden">
                      <button
                        ...
                        onClick={() =>
                          Updata(item?.product.id, item.count - 1, "-")
                        }
                      ...
                      {loadingData && item.product.id === loadingCurrnet ? (
                        <Loader2
                          className="animate-spin text-green-600 mx-2"
                          size={25}
                        />
                      ) : (
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-center pt-6">
                <Link
                  href="/"
                  className="text-green-600 font-bold flex items-center gap-2 hover:underline transition-all order-2 sm:order-1"
                >
                  ←{" "}
                  ...
                  <div className="py-6 space-y-6">
                    ...
                    <p className="text-center text-sm text-gray-600">
                      ...
                      <span className="text-gray-400">
                        Secure checkout powered by Route
                      </span>
                    </p>
                  </div>
        ...
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4 animate-in fade-in zoom-in duration-500">
          ...
          <div className="max-w-md space-y-4">
            ...
            <p className="text-gray-500 font-medium text-lg leading-relaxed">
              ...
              our latest deals and find something you love!
            </p>
          </div>
          ...
          <div className="mt-12 flex gap-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className={`w-2 h-2 rounded-full bg-gray-200 animate-pulse [animation-delay:${i * 200}ms]`}
              />
            ))}
          </div>
        </div>
